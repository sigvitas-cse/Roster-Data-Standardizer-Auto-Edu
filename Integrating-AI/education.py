    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if "429" in msg or "Resource exhausted" in msg or "RateLimit" in msg:
                ...
                # Default fallback sleep time
                wait_time = SLEEP_SECONDS * 2  # A safe default if parsing fails

                # Try to extract the recommended retry delay (e.g., "retry in 33.100131018s")
                ...
                if delay_match:
                    try:
                        ...
                        logging.warning("Failed to parse retry delay from error message.")
                
                print(f"Rate limit detected. Waiting for {wait_time:.1f} seconds to comply with API request.")
                logging.info("Pausing for %s seconds due to rate limit.", wait_time)
                time.sleep(wait_time)

                # Instead of 'break', we now use 'continue' to skip the
                # logging/manual review for this entry and try the same one again.
                # To retry the *same* person, you must decrement idx and daily_calls,
                # or simply use 'continue' and rely on the next loop iteration (which will use the next name).
                
                # **RECOMMENDATION:** The simplest change is to change `break` to `continue` and decrement `idx` to retry the *same* name.
                # However, since this whole call is ALREADY wrapped in a tenacity retry decorator, 
                # it's better to let tenacity handle the retry.
                # If the exception is NOT a retryable one for tenacity (which it is, since you catch 'Exception'), 
                # you should re-raise.

                # For a clean stop-and-resume logic without relying on tenacity:
                # print("Rate limit detected; **stopping** to allow resume later.")
                print("Rate limit resolved. Retrying this entry.")
                continue
